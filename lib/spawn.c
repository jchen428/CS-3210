...
static int init_stack_exec(envid_t child, const char **argv, uintptr_t *init_esp);
static int map_segment(envid_t child, uintptr_t va, size_t memsz,
                       int fd, size_t filesz, off_t fileoffset, int perm);
static int copy_shared_pages(envid_t child);
extern int sys_exec(uint32_t eip, uint32_t esp, void *_ph, uint32_t phnum);
...
exec(const char *prog, const char **argv)
{
  ...
  for (i = 0; i < elf->e_phnum; i++, ph++) {
    ...
    if (ph->p_flags & ELF_PROG_FLAG_WRITE)
      perm |= PTE_W;
    if ((r = map_segment(0, tempRegion + PGOFF(ph->p_va), ph->p_memsz,
                         fd, ph->p_filesz, ph->p_offset, perm)) < 0)
      goto error;

    tempRegion += ROUNDUP(ph->p_memsz + PGOFF(ph->p_va), PGSIZE);
  }
  close(fd);
  fd = -1;

  if ((r = init_stack_exec(0, argv, &esp)) < 0)
    panic("init_stack_exec() in exec() failed");
  else if (sys_exec(elf->e_entry, esp, (void *)(elf_buf + elf->e_phoff), elf->e_phnum) < 0)
    panic("sys_exec() in exec() failed");
...
static int
init_stack_exec(envid_t child, const char **argv, uintptr_t *init_esp)
{
  size_t string_size;
  int argc, i, r;
  char *string_store;
  ...
  *init_esp = UTEMP2USTACK(&argv_store[-2]);
  ...
  // After completing the stack, map it into the child's address space
  // and unmap it from ours!
  if ((r = sys_page_map(0, UTEMP, child, TEMPEXEC, PTE_P | PTE_U | PTE_W)) < 0)
    goto error;
  if ((r = sys_page_unmap(0, UTEMP)) < 0)
    goto error;
...
copy_shared_pages(envid_t child)
{
  ...
  for (va = 0; va < UTOP; va += PGSIZE)
    ...
    else if ((uvpt[PGNUM(va)] & (PTE_P|PTE_SHARE)) == (PTE_P|PTE_SHARE)) {

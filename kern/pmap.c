...
mem_init(void)
{
  ...
  //memset(kern_pgdir, 0, sizeof(struct PageInfo) * npages);
  // Makes line 147 useless? Causes check_kern_pgdir() to fail permissions.
  ...
  // Permissions: kernel RW, user NONE
  // Your code goes here: -- DONE
  boot_map_region(kern_pgdir, KERNBASE, 0x100000000 - KERNBASE, 
    0, PTE_P | PTE_W);
...
mem_init_mp(void)
{
  ...
  uint32_t i = 0;
  uint32_t currStackFloor = KSTACKTOP-KSTKSIZE;

  for (i; i < NCPU; i++) {
    boot_map_region(kern_pgdir, currStackFloor, KSTKSIZE, 
      PADDR(percpu_kstacks[i]),  PTE_P | PTE_W);
    currStackFloor -= (KSTKGAP + KSTKSIZE);
  }
}
...
page_init(void)
{
  ...
  // as in use
      ...
      //
      // Change the code to reflect this. -- DONE
  ...
  for (i = 0; i < npages; i++) {
    if (i == 0 || (i >= IOPHYSMEM / PGSIZE && i < EXTPHYSMEM / PGSIZE) ||
        (i >= EXTPHYSMEM / PGSIZE && 
        i < (EXTPHYSMEM - KERNBASE + ((uint32_t)boot_alloc(0))) / PGSIZE) ||
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  int pageExists = *pde & PTE_P;

  if (!pageExists && !create) {
    return NULL;
  } else if (!pageExists && create) {
...
{
  // LAB 3: Your code here. -- DONE
  uint32_t curr = (uint32_t)ROUNDDOWN(va, PGSIZE);
  uint32_t end = (uint32_t)ROUNDUP(va + len, PGSIZE);

  pte_t *page;
  for (curr; curr < end; curr += PGSIZE) {
    page = pgdir_walk(env->env_pgdir, (void *)curr, 0);

    if (!page || curr >= ULIM || ((*page & (perm | PTE_P)) != (perm | PTE_P))) {
      if (curr == (uint32_t)ROUNDDOWN(va, PGSIZE))

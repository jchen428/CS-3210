...
sys_exofork(void)
{
  ...
  struct Env *env;
  int retcode = env_alloc(&env, curenv->env_id);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
  ...
  if (envid2env(envid, &env, 1) < 0) {
    return -E_BAD_ENV;
  } else if ((int)va >= UTOP || (int)va % PGSIZE != 0) {
    return -E_INVAL;
  } else if ((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P) || (perm & ~PTE_SYSCALL) != 0) {
    return -E_INVAL;
  } else if (page_insert(env->env_pgdir, page, va, perm) < 0) {
    page_free(page);
    return -E_NO_MEM;
  } else {
    return 0;
             ...
             envid_t dstenvid, void *dstva, int perm)
{
  ...
  if (envid2env(srcenvid, &srcEnv, 1) < 0 || envid2env(dstenvid, &dstEnv, 1) < 0) {
    return -E_BAD_ENV;
  } else if ((int)srcva >= UTOP || (int)srcva % PGSIZE != 0 || 
      (int)dstva >= UTOP || (int)dstva % PGSIZE != 0) {
    return -E_INVAL;
  } else if (!page_lookup(srcEnv->env_pgdir, srcva, 0)) {
    return -E_INVAL;
  } else if ((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P) || 
      (perm & ~PTE_SYSCALL) != 0) {
    return -E_INVAL;
  } else if (!(page = page_lookup(srcEnv->env_pgdir, srcva, &pte)) && 
      (perm & PTE_W) == PTE_W && 
      (*pte & PTE_W) == 0) {
    return -E_INVAL;
  } else if (page_insert(dstEnv->env_pgdir, page, dstva, perm) < 0) {
    page_free(page);
    return -E_NO_MEM;
  } else {
    return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
  ...
  if (envid2env(envid, &env, 0) < 0) {
    ...
    if ((int)srcva % PGSIZE != 0) {
      ...
      return -E_INVAL;
    } else if (!(page = page_lookup(curenv->env_pgdir, srcva, &pte))) {
      return -E_INVAL;

...
trap_init(void)
{
  ...
  SETGATE(idt[IRQ_OFFSET + IRQ_ERROR], 0, GD_KT, handlers[38], 0);
...
page_fault_handler(struct Trapframe *tf)
{
  ...
  if (curenv->env_pgfault_upcall) {
    ...
    if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP)
      utfStart = tf->tf_esp - sizeof(struct UTrapframe) - 4;
    else
      utfStart = UXSTACKTOP - sizeof(struct UTrapframe);
    
    user_mem_assert(curenv, (void *)utfStart, sizeof(struct UTrapframe), PTE_W);

    utf = (struct UTrapframe *)utfStart;

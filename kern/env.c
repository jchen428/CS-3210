...
env_setup_vm(struct Env *e)
{
  ...
  memcpy(e->env_pgdir, kern_pgdir, 4 * NPDENTRIES);
...
region_alloc(struct Env *e, void *va, size_t len)
{
  ...
  void *curr = ROUNDDOWN(va, PGSIZE);
  void *end = ROUNDUP(va + len, PGSIZE);
  struct PageInfo *page;

  for (curr; curr < end; curr += PGSIZE) {
    page = page_alloc(0);

    if (!curr || !end || !page)
      panic("region_alloc: environment region allocation failed");

    if (page_insert(e->env_pgdir, page, curr, PTE_W | PTE_U) != 0)
